    ...
    public static void main(String[] args) {
        ...
        collection
            .aggregate(
                ...
                Arrays.asList(
                    Aggregates.match(
                        Filters.in("color", "red", "blue")), // allow only red and blue shapes to pass
                    Aggregates.addFields(
                        new Field<>("shape", "circle")), // change shapes to circle
                    Aggregates.group(
                        0, // group all incoming documents together
                        Accumulators.sum(
                            "red",
                        ...
                        Accumulators.sum(
                            "blue",
                            new Document(
                                "$cond", // if the color is blue, add 1 to the new "blue" field
                                Arrays.asList(
                                    new Document("$eq", Arrays.asList("$color", "blue")), 1, 0)))))
                // end aggregation
            )
