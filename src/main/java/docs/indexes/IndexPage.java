...
public class IndexPage {
    ...
    private void wildCardIndex() {
        ...
        // begin compound index
        String resultCreateIndex = collection.createIndex(Indexes.ascending("tomatoes.$**"));
        System.out.println(String.format("Index created: %s", resultCreateIndex));
        // end compound index

        // begin covered compound query
        Bson filter = gt("tomatoes.viewer.rating", 3.5);
        Bson sort = Sorts.descending("tomatoes.viewer.rating");
        Bson projection = fields(include("title", "tomatoes.viewer.rating"), excludeId());
        FindIterable<Document> cursor = collection.find(filter).sort(sort).projection(projection);
        // end covered compound query
        cursor.forEach(doc -> System.out.println(doc));
    }
